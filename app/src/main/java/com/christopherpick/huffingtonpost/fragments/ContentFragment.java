...
public class ContentFragment extends Fragment implements LoaderManager.LoaderCallbacks<Cursor> {
	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
		...
		if (cursor.getCount() == 1) {
			String title = cursor.getString(cursor.getColumnIndexOrThrow(HuffingtonPostContract.COLUMN_TITLE));
			String author = cursor.getString(cursor.getColumnIndexOrThrow(HuffingtonPostContract.COLUMN_AUTHOR_NAME));
	//		String authorUri = cursor.getString(cursor.getColumnIndexOrThrow(HuffingtonPostContract.COLUMN_AUTHOR_URI));
			String content = cursor.getString(cursor.getColumnIndexOrThrow(HuffingtonPostContract.COLUMN_CONTENT));
			String publishedDate = cursor.getString(cursor.getColumnIndexOrThrow(HuffingtonPostContract.COLUMN_PUBLISHED_DATE));
			String editedDate = cursor.getString(cursor.getColumnIndexOrThrow(HuffingtonPostContract.COLUMN_UPDATED_DATE));
			mArticleUrl = cursor.getString(cursor.getColumnIndexOrThrow(HuffingtonPostContract.COLUMN_ARTICLE_LINK));
			int isFavorite = cursor.getInt(cursor.getColumnIndexOrThrow(HuffingtonPostContract.COLUMN_FAVORITE));
